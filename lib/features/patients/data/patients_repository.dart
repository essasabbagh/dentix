...
class PatientsRepository {
  ...
  PatientsTableCompanion _toCompanion(PatientModel m) => PatientsTableCompanion(
    id: Value(m.id),
  ...
  Stream<List<PatientModel>> watchAllPatients() => _db.patientsDao
      .watchAllPatients()
      .map((rows) => rows.map(_fromData).toList());

  Stream<List<PatientModel>> watchSearchPatients(String query) => _db
      .patientsDao
  ...
  Stream<PatientModel?> watchPatientById(int id) => _db.patientsDao
      .watchPatientById(id)
      .map((d) => d != null ? _fromData(d) : null);
